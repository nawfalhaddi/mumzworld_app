...
if (!screenName) {
  ...
  console.log(
    '\nThis screen exists already. Please remove it from ./src/navigation/routesNames.ts and ./src/types/navigation.ts \n',
  );
} else {
  ...
  fs.mkdirSync(screenPath + `components/${screenName}View`, {
    recursive: true,
  });
